            ...
            <ul className="divide-y divide-gray-200">
                {disputes.length === 0 ? (
                    ...
                    disputes.map((dispute) => (
                        <li key={dispute.id}>
                            <Link href={`/disputes/${dispute.id}`} className="block hover:bg-gray-50">
                                <div className="px-4 py-4 sm:px-6">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="ml-2 flex-shrink-0 flex">
                                            <p className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${dispute.status === 'OPEN' ? 'bg-green-100 text-green-800' :
                                    ...
                                    <div className="mt-2 sm:flex sm:justify-between">
                                        ...
                                        <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                                            ...
                                            {/* Seller: Action needed if OPEN */}
                                            {/* Admin: Action needed if SELLER_RESPONDED (or maybe OPEN too depending on flow) */}
                                        </div>
                                    </div>
                                </div>
                            </Link>
